...
const UploadZone = ({ onUpload }) => {
  ...
  const processFile = async (file) => {
    ...
    if (file.size > 100 * 1024 * 1024) {
      setError(`File size exceeds the 100MB limit (${(file.size / (1024 * 1024)).toFixed(1)}MB). Please upload a smaller file.`);
      return;
    }
    ...
    <div className="w-full">
      ...
      >
        ...
        <p className="text-gray-400 text-sm mb-4 text-center max-w-md">
          Supports MP3, WAV, M4A, AAC, MP4, WebM, MOV. Max file size: 100MB.
        </p>
